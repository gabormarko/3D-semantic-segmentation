...
def prompts_to_gaussian_onehot(
    ...
    # Assign color to each voxel
    voxel_colors = np.zeros((N, 3), dtype=np.uint8)
    for i in range(N):
        idx = int(voxel_cls[i])
        voxel_colors[i, 0] = palette[3*idx+0]
        voxel_colors[i, 1] = palette[3*idx+1]
        voxel_colors[i, 2] = palette[3*idx+2]

    print(f"[DEBUG] Assigned palette colors to voxels. Example colors: {voxel_colors[:5]}")

    return voxel_cls.cpu().numpy(), voxel_logits.cpu().numpy(), voxel_colors
...
def _cli_query(args: argparse.Namespace) -> None:
    voxel_pos, voxel_feat = load_voxels(args.vox)
    labels, logits, voxel_colors = prompts_to_gaussian_onehot(
        args.prompt, voxel_feat, None, device=args.device
    )
    # Save both label indices, logits, colors, and prompt list in output
    np.savez(args.out, labels=labels, logits=logits, prompts=np.array(args.prompt), colors=voxel_colors)
    print(f"[✓] Voxel labels, logits, and colors saved: {args.out}  positives={np.sum(labels)}/{labels.size}")
    ...
    # --- Save colored .ply ---
    ply_path = str(args.out).replace('.npz', '_colored_voxels.ply')
    with open(ply_path, 'w') as f:
        f.write('ply\nformat ascii 1.0\n')
        f.write(f'element vertex {voxel_pos.shape[0]}\n')
        f.write('property float x\nproperty float y\nproperty float z\n')
        f.write('property uchar red\nproperty uchar green\nproperty uchar blue\n')
        f.write('end_header\n')
        for i in range(voxel_pos.shape[0]):
            x, y, z = voxel_pos[i].tolist()
            r, g, b = voxel_colors[i].tolist()
            f.write(f'{x} {y} {z} {r} {g} {b}\n')
    print(f"[✓] Colored voxel .ply saved: {ply_path}")
...
if __name__ == "__main__":
    ...
    qry = sp.add_parser("query", help="Prompt → per-voxel labels and coloring")
    ...
    qry.add_argument("--out", type=pathlib.Path, required=True,
                     help=".npz output with labels, logits, colors")
