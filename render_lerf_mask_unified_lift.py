...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, classifier, groundingdino_model, sam_predictor, scene_name,TEXT_PROMPT, threshold=0.2):
    render_path = os.path.join(model_path, name, "ours_gg_{}_text_test".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "ours_gg_{}_text_test".format(iteration), "gt")
    colormask_path = os.path.join(model_path, name, "ours_gg_{}_text_test".format(iteration), "objects_feature16")
    pred_obj_path = os.path.join(model_path, name, "ours_gg_{}_text_test".format(iteration), "test_mask")
    pred_obj_numpy = os.path.join(model_path, name, "ours_gg_{}_text_test".format(iteration), "prediction_numpy")
    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)
    makedirs(colormask_path, exist_ok=True)
    makedirs(pred_obj_path, exist_ok=True)
    makedirs(pred_obj_numpy, exist_ok=True)
    ...
    rendering_obj0 = rendering_obj0 #/ (torch.norm(rendering_obj0, dim=0, keepdim=True) + 1e-6).detach()
    # logits = classifier(rendering_obj0)
    # pred_obj = torch.argmax(logits,dim=0)
    H,W = rendering_obj0.shape[1], rendering_obj0.shape[2]
    
    Flag_classifier = False
    if Flag_classifier:
        logits = classifier(rendering_obj0)
    else:
        rendering_obj = rendering_obj0.reshape(16, -1)
        logits = torch.matmul(classifier, rendering_obj) 
        # pred_obj = torch.argmax(predict, dim=1)
        
        logits = logits.reshape(256, H,W)
        rendering_obj = rendering_obj.reshape(16, H, W)
    # predict = torch.matmul(feature_obj, weight)
    pred_obj = torch.argmax(logits,dim=0)

    ### load RGB from gaussian group same results, note that this operation is designed only for fair metric evaluation as Gaussian Grouping.
    # 

    gg_render_path = f"/research/d1/gds/rszhu22/Gaussian-Grouping/checkpoint/lerf_mask/{scene_name}/test/ours_30000_text/renders/00000.png"
    image = np.array(Image.open(gg_render_path)).astype('uint8')
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering_obj = rendering_obj #/ (torch.norm(rendering_obj, dim=0, keepdim=True) + 1e-6).detach()
        ...
        if Flag_classifier:
            ...
            rendering_obj = rendering_obj.reshape(16, -1)
            logits = torch.matmul(classifier, rendering_obj) 
            # pred_obj = torch.argmax(predict, dim=1)
        ...
        # all_pre_obj.append(pred_obj)
        Image.fromarray(pred_obj_mask).save(os.path.join(pred_obj_path, '{0:05d}'.format(idx) + ".png"))
        
        ### save numpy 
        np.save(os.path.join(pred_obj_numpy, '{0:05d}'.format(idx) + ".npy"),pred_obj.cpu().numpy().astype(np.uint8))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        classifier = (torch.load(os.path.join(dataset.model_path,"point_cloud","iteration_"+str(scene.loaded_iter),"classifier.pth")))
        ...
        for TEXT_PROMPT in positives:
            if not skip_train:
                render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, classifier, groundingdino_model, sam_predictor,scene_name, TEXT_PROMPT)
            if not skip_test:
                render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, classifier, groundingdino_model, sam_predictor,scene_name, TEXT_PROMPT)
