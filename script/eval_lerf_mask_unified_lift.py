...
parser.add_argument('--excel_name',type=str, help='')
parser.add_argument('--pred_path', type=str, help='')

args = parser.parse_args()
excel_name = args.excel_name
pred_path = args.pred_path

all_dataset_list = ["figurines","ramen", "teatime"]
evaluation_txt_file = open(f"{excel_name}.csv", "w")
evaluation_txt_file.write(f"dataset_name,overall_mean_iou,overall_mean_biou\n")
...
for dataset_name in all_dataset_list:
    ...
    gt_folder_path = os.path.join('/research/d1/gds/rszhu22/gaussian-grouping/data/lerf',dataset_name,'test_mask')
    # You can change pred_folder_path to your output
    # ${scenes[$index]}
    
    
    pred_folder_path = os.path.join(f'{pred_path}/{dataset_name}/', 'test/ours_gg_30000_text_test/test_mask')
    
    
    
    print(pred_folder_path)
    # exit()
    ...
    # evaluation_txt_file = open("2D_GS_segment.csv", "a")
    evaluation_txt_file.write(f"{dataset_name},{overall_mean_iou},{overall_mean_biou}\n")
    evaluation_txt_file.flush()
    
    mean_iou_list.append(overall_mean_iou)
    mean_biou_list.append(overall_mean_biou)


mean_mean_iou = np.array(mean_iou_list).mean()
mean_mean_biou = np.array(mean_biou_list).mean()
evaluation_txt_file.write(f"avarage,{mean_mean_iou},{mean_mean_biou}\n")
evaluation_txt_file.flush()